...
export function trialRow(trialId: number, total: number, reward: number, duration: string, commands: number, scorers: { type: string; score: number }[]) {
    const pad = '  ';
...
export function validationResult(passed: boolean, reward: number, scorers: { type: string; score: number; details: string }[]) {
