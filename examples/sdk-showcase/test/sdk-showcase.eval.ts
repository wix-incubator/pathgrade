/**
 * SDK Showcase — a real shared-run eval that demonstrates the main SDK surfaces.
 *
...
const DEBUG_ROOT = path.join(__dirname, 'pathgrade-debug');
const hookTimeoutMs = 300_000;
const SHOWCASE_AGENT = resolveShowcaseAgent();
// claude is noninteractive (policy applied); codex/app-server is reliable (policy absent).
const EXPECTS_NONINTERACTIVE_POLICY = SHOWCASE_AGENT === 'claude';
...
function resolveShowcaseAgent(): 'claude' | 'codex' {
    return process.env.PATHGRADE_AGENT === 'claude' || process.env.PATHGRADE_AGENT === 'codex'
        ? process.env.PATHGRADE_AGENT
        : 'codex';
}
...
async function getShowcaseRun(): Promise<ShowcaseRun> {
    ...
    showcaseRunPromise = (async () => {
        ...
        const snapshotReplay = await evaluate.fromSnapshot(snapshotPath, [
            check('runtime-policy-matches-transport', ({ log }) => {
                const recorded = log.some((entry) =>
                    entry.runtime_policies_applied?.some(
                        (policy) => policy.id === 'noninteractive-user-question',
                    ),
                );
                return recorded === EXPECTS_NONINTERACTIVE_POLICY;
            }),
...
// Default resolves to codex; skip on CI where codex has no valid OpenAI auth.
describe.skipIf(SHOWCASE_AGENT === 'codex' && !!process.env.CI)('sdk-showcase: shared bugfix flow', () => {
    ...
    it('captures the run as a snapshot and previews reactions offline', async () => {
        ...
        ).toBe(EXPECTS_NONINTERACTIVE_POLICY);
