/**
 * SDK Showcase (Cursor) — a real shared-run eval that demonstrates the main SDK surfaces.
 *
...
const SHOWCASE_AGENT = 'cursor' as const;
...
// Cursor CLI uses keychain auth, not an env key, so skipping on missing
// CURSOR_API_KEY hid the suite locally too. Skip only on CI, where the
// cursor CLI isn't provisioned.
describe.skipIf(!!process.env.CI)('sdk-showcase (cursor): shared bugfix flow', () => {
