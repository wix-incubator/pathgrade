/**
 * SDK Showcase (Codex) — a real shared-run eval that demonstrates the main SDK surfaces.
 *
...
const SHOWCASE_AGENT = 'codex' as const;
...
async function getShowcaseRun(): Promise<ShowcaseRun> {
    ...
    showcaseRunPromise = (async () => {
        ...
        const snapshotReplay = await evaluate.fromSnapshot(snapshotPath, [
            check('runtime-policy-absent-for-reliable-transport', ({ log }) =>
                !log.some((entry) =>
...
describe.skipIf(!!process.env.CI)('sdk-showcase (codex): shared bugfix flow', () => {
    ...
    it('captures the run as a snapshot and previews reactions offline', async () => {
        ...
        // codex/app-server exposes a reliable interactive-question transport,
        // so the noninteractive runtime policy should NOT be applied.
        expect(
