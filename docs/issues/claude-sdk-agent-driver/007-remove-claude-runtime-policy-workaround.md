...
- [x] Claude session creation no longer prepends non-interactive runtime-policy text to first-turn prompts.
- [x] Claude `AgentTurnResult.runtimePoliciesApplied` is preserved as an empty array.
- [x] Codex exec and Cursor retain the existing non-interactive runtime-policy behavior and preflight protection.
- [x] Conversation-time preflight treats Claude ask-user reactions as reachable because its transport is reliable.
- [x] Tests assert Claude no longer receives the runtime-policy prompt and that Codex/Cursor behavior is unchanged.
